...
``No'' is among the first words that children learn, as well as one of the most important.  Negation is a fundamental element of human language---it is essential to logical systems, allows us to evaluate whether a statement is true or false, and it gives us a way to express concepts such as nonexistence.  But negation is also challenging, even for adult language users, who take longer to process negative sentences than positive ones \cite{hclark1972}.  These findings lead to an apparent paradox: How is it that negation is difficult for adults, yet acquired at such a young age?  By examining children's comprehension of negation, we can explore the challenges that children may face in mastering this important component of language.  
...
{\it Conceptual factors.} Nonexistence and refusal may be less complex concepts than denial (though of course conceptual complexity is very difficult to measure). Understanding denial requires evaluating a proposition and determining whether it is true or false, potentially making it more challenging than either perceiving the absence of an expected object or refusing an unappealing object or action.

{\it Pragmatic/contextual factors.} Nonexistence and refusal might occur in contexts that are more salient to young children. According to Grice's Cooperative Principle, speakers should produce utterances that are informative given the context \cite{Grice1975}.  Negative sentences are most informative when the negate some expectation that a listener might have.  For example, a child might reach into a cookie jar, expecting a cookie, and exclaim ``no cookies!'' upon finding that the jar is empty.  Both nonexistence and refusal often occur in feeding situations  (e.g. ``no more juice!''; ``all gone!''; ``don't want!''), where food or drink has recently disappeared or where something is being offered to the child that they do not want.  In these contexts, expectations may be more salient, making negation more natural to produce.  Denial typically negates expectations that may be harder for a two-year-old to identify (e.g. another person's beliefs about the world). Early denials often occur in contexts where such an expectation/belief has been made explicit, such as when an adult deliberately mis-labels an object in the context of a yes/no question (e.g. Adult: ``Is that a biscuit?'' Child: ``No, apple'', \citeNP{pea1980, pea1980b, pea1982}).  Thus, the different types of negation seen in children's early speech may have more to do with the contexts in which these negation typically occur; opportunities to express refusal and nonexistence may simply occur more frequently in a two-year-old's day-to-day life.  


%{\it Cognitive factors.} One previously unexamined factor in children's negation comprehension is the cognitive ability required in contexts where negation is meant to inhibit a response. For example, an adult might say ``don't touch'' to a child who is reaching towards a dangerous object.  A correct response to this phrase requires children to intervene on their own behavior and stop the action they are currently engaged in.  This type of response requires substantial \emph{inhibitory control}, defined as the ability to inhibit irrelevant stimuli or overcome a prepotent response while pursuing a goal. Although the broader set of abilities involved in inhibitory control continues to mature over the course of later childhood and adolescence, even early childhood is a time during which there are tremendous changes in inhibitory abilities \cite{diamond1996,zelazo1997,davidson2006,garon2008}. Perhaps these changes play a role in children's ability to produce and process different types of negation. Below we review evidence on adults' comprehension of negation, suggesting that even expert language users incur a processing cost with negative utterances, perhaps in part due to the same inhibitory demands.
...
While some work suggests that children are sensitive to these contextual effects as well, few studies have tested the effect of context on children's understanding of negation.  This body of evidence suggests that in addition to linguistic and conceptual factors, the pragmatic explanation given above may play a role in the order of children's acquisition of negation.
...
All data were exported from the SMI format to simple text files and processed using the R data analysis language. Data from blinks and other cases where track was lost were treated as missing and excluded from further analysis. Each trial was synchronized to the point of disambiguation, which was coded individually for each audio stimulus. The point of disambiguation for both positive and negative trials was defined as the onset of the target noun (e.g. ``apples'').  Although it is possible in this experiment that participants could guess that ``no'' refers to the boy with nothing, hearing the target noun is necessary to confirm what is being negated. Looks were coded as being on the correct target if they fell on the corresponding side of the screen (excluding a  400 pixel zone in the middle of the two targets).  
...
Previous work with adults has shown delayed comprehension of negative sentences compared to positive sentences \cite{hclark1972, just1971, just1976, carpenter1975}.  On our task, however, adults appeared to show comparable performance on both positive and negative sentences.  Although adults showed a slight preference for the character with nothing early in the trial (discussed below), they actually appear to shift their gaze to the target of negative sentences faster than they shift their gaze to the target of positive sentences (as indicated by the sharper change in slope at the disambiguating noun).  In fact, given that it takes 200 ms for adults to plan a saccade (CITE??), adults appear to be identifying the target of negative sentences as soon as they hear the word ``no''.  Unlike in previous studies of adults' negation processing, in our stimuli there is an obvious referent for the negation (e.g. the character with nothing).  It is possible that adults are making a prediction about which character is the referent of a negative sentence, even before they hear the disambiguating noun.   
...
\begin{tabular}{rrrr}
 ...
 & Coefficient  & Std. Err. & $t$ value \\ 
  \hline
Intercept & 0.77 & 0.03 & 25.14 \\ 
  Age (Three-year-olds) & 0.00 & 0.04 & 0.01 \\ 
  Age (Four-year-olds) & 0.06 & 0.04 & 1.29 \\ 
  Age (Adults) & 0.16 & 0.04 & 3.93 \\ 
  Sentence (Negative) & -0.51 & 0.05 & -10.96  \\ 
  Age (Three-year-olds) $\times$ Sentence (Negative) & 0.22 & 0.06 & 3.77 \\ 
  Age (Four-year-olds) $\times$ Sentence (Negative) & 0.33 & 0.06 & 5.31 \\ 
  Age (Adults) $\times$ Sentence (Negative) & 0.45 & 0.07 & 6.77  \\ 
   \hline
\end{tabular}
\end{center}
\end{table}
...
To examine the reliability of our findings, we fit a linear mixed-effects model to dwell times on the target pictures. In particular, we analyzed the interaction between sentence type and age group on each participant's proportion looking to the target between 600 and 2600ms following the onset of the noun for each trial. Coefficients for this model are shown in Table \ref{tab:e1model}.\footnote{Because of the robustness of the patterns we observed, for simplicity and interpretability we fit a linear model to proportions of looking within a target window rather than attempting to fit a series of logistic functions for individual time points. All mixed-effects models were fit using the lme4 package in R version 2.15.2.  The model specification was as follows: \texttt{on.target $\sim$ age~$\times$~sentence + (sentence~\textbar~subject) +  (sentence + age~\textbar~item)}.  Significance was calculated using the standard normal approximation to the $t$ distribution \cite{barr2013}.} Results of this model indicate a highly significant effect of sentence type, with significantly fewer children looking to the target picture in response to negative sentences compared to positive sentences ($\beta= -.51$, $p< .001$).   There were significant interactions between age group and sentence type for three-year-olds ($\beta = .22$, $p< .001$), four-year-olds ($\beta = .33$, $p< .001$), and adults ($\beta = .45$, $p< .001$), indicating that the increase in accuracy across development is driven primarily by improved comprehension of negative sentences.  

Together, these results show an interesting developmental pattern. Two-year-olds seemed to disregard the negative element completely, treating the negative sentences as positive, and three-year-olds were at chance in their responses to negative sentences.  Only by age 4 were children able to begin identifying the correct referent of the negative items.  One possible explanation for the poor performance of two- and three-year-old children is that this is an unusual context for nonexistence negation.  Typically nonexistence negation is produced in contexts where a salient object has disappeared, or when an expected object or action has failed to occur.  This context required children to orient \emph{away} from the more interesting picture.  Perhaps three-year-olds remained at chance in part because they knew that they \emph{should} look to the target, but they had difficulty shifting their attention away from the more salient picture.  On this account, children have the conceptual knowledge necessary to understand negation, but the context is challenging for children because it requires them to inhibit their desire to look at the more interesting picture. If this is the case, children should show increased comprehension of negation in contexts where the possible targets of negation are equally salient.

\section{Experiment 2}

Experiment 1 found that children up to age four struggled to identify the referent of a negative sentence when that sentence referred to ``nothing.''  Because nonexistence negation is one of the first types of negation that children produce, it seems unlikely that this difficulty was due to lack of conceptual understanding.  A more likely explanation is that negative sentences in Experiment 1 required children to look at the character holding nothing, who was considerably less interesting.  When young children in naturalistic settings produce nonexistence negation, they are typically referring to an object that has recently disappeared from the scene or expressing surprise that an object is not in its expected location \cite{pea1980}.  In addition, orienting away from a more interesting stimulus may require a level of control that two- and three-year-olds are not capable of.  In Experiment 2, we modified our stimuli so that the target of the negative sentences is holding a comparably interesting item that does not match the target noun (e.g. a boy holding presents on ``apples'' trials).  
...
\begin{table}
\caption{\label{tab:e2model}Coefficient estimates from a mixed-effects model predicting proportion of looks to target in an early window (600-1600 ms after noun onset) and a late window (1600-2600 ms after noun onset) (Experiment 2).}
\begin{center}
\small\addtolength{\tabcolsep}{-5pt}
\begin{tabular}{rrrr}
  \hline
 & Coefficient  & Std. Err. & $t$ value \\ 
  \hline
Intercept & 0.63 & 0.03 & 19.42 \\ 
  Age (Three-year-olds) & 0.09 & 0.05 & 1.90 \\ 
  Age (Four-year-olds) & 0.11 & 0.04 & 2.52 \\ 
  Age (Adults) & 0.25 & 0.05 & 5.22 \\ 
  Sentence (Negative) & -0.31 & 0.05 & -6.40 \\ 
  Window (Late) & 0.04 & 0.04 & 1.03 \\ 
  Age (Three-year-olds)$\times$Sentence (Negative) & -0.04 & 0.06 & -0.70 \\ 
  Age (Four-year-olds)$\times$Sentence (Negative) & -0.01 & 0.06 & -0.19 \\ 
  Age (Adults)$\times$Sentence (Negative) & 0.16 & 0.07 & 2.29 \\ 
  Age (Three-year-olds)$\times$Period (Late) & -0.04 & 0.05 & -0.77 \\ 
  Age (Four-year-olds)$\times$Period (Late) & -0.11 & 0.06 & -2.01 \\ 
  Age (Adults)$\times$Period (Late) & 0.01 & 0.06 & 0.09 \\ 
  Sentence (Negative)$\times$Period (Late) & -0.02 & 0.05 & -0.37 \\ 
  Age (Three-year-olds)$\times$Sentence (Negative)$\times$Period (Late) & 0.25 & 0.07 & 3.52 \\ 
  Age (Four-year-olds)$\times$Sentence (Negative)$\times$Period (Late) & 0.37 & 0.07 & 4.89 \\ 
  Age (Adults)$\times$Sentence (Negative)$\times$Period (Late) & 0.12 & 0.08 & 1.56 \\ 
   \hline
\end{tabular}
\end{center}
\end{table}

We again fit a linear mixed-effects model to explore the interaction between sentence type and age group. In this case, since we observed such striking differences in time-course, we added a third predictor to our model: time window (Table \ref{tab:e2model}).\footnote{The model specification was \texttt{on.target $\sim$ age~$\times$~sentence~$\times$~window + (sentence + window~\textbar~subject) +  (sentence + age + window~\textbar~item)}.} We defined two windows based on the pattern of data seen in Figure \ref{fig:e2simple}, an early window spanning from 600 to 1600 ms following the point of disambiguation, and a late window, spanning from 1600 to 2600 ms following the point of disambiguation. 

Results of this model indicate a significant effect of sentence type, with significantly fewer participants looking to the target picture in response to negative sentences compared to positive sentences ($\beta = -.31$, $p < .001$).   There were significant 3-way interactions between age group, sentence type, and time window for three-year-olds ($\beta = .25$, $p  < .001$) and four-year-olds ($\beta = .37$, $p  < .001$), indicating that in response to negative sentences, three- and four-year-olds showed an increase in looks to the target specifically in the later window of time.  This three-way interaction was not significant for adults ($\beta = .13$, $p = .13$).  


% latex table generated in R 2.15.3 by xtable 1.7-1 package
% Wed Jun 26 23:18:35 2013
\begin{table}
\caption{\label{tab:bothmodel}Coefficient estimates from a mixed-effect model predicting proportion of looks to target in an early window (600-1600 ms after noun onset) and a late window (1600-2600 ms after noun onset), with Experiment as an additional factor.}
\begin{center}
\small\addtolength{\tabcolsep}{-5pt}
\begin{tabular}{rrrr}
  \hline
 & Coefficient  & Std. err & $t$ value \\ 
  \hline
Intercept & 0.83 & 0.03 & 26.32 \\ 
  Age (Three-year-olds) & -0.04 & 0.04 & -1.08 \\ 
  Age (Four-year-olds)& 0.04 & 0.04 & 0.84 \\ 
  Age (Adults) & 0.10 & 0.05 & 2.18 \\ 
  Sentence (Negative) & -0.58 & 0.05 & -11.72 \\ 
  Window (Late)  & -0.11 & 0.04 & -2.94 \\ 
  Experiment (Exp. 2) & -0.19 & 0.04 & -4.58 \\ 
  Age (Three-year-olds)$\times$Sentence (Negative) & 0.30 & 0.06 & 4.76 \\ 
  Age (Four-year-olds)$\times$Sentence (Negative) & 0.39 & 0.07 & 5.79 \\ 
  Age (Adults)$\times$Sentence (Negative) & 0.50 & 0.07 & 6.94 \\ 
  Age (Three-year-olds)$\times$Window (Late)  & 0.07 & 0.05 & 1.45 \\ 
  Age (Four-year-olds)$\times$Window (Late) & 0.05 & 0.06 & 0.82 \\ 
  Age (Adults)$\times$Window (Late)  & 0.12 & 0.06 & 2.14 \\ 
  Sentence (Negative)$\times$Window (Late)  & 0.15 & 0.05 & 2.97 \\ 
  Age (Three-year-olds)$\times$Experiment (Exp. 2)  & 0.13 & 0.06 & 2.28 \\ 
  Age (Four-year-olds)$\times$Experiment (Exp. 2)  & 0.07 & 0.06 & 1.23 \\ 
  Age (Adults)$\times$Experiment (Exp. 2) & 0.15 & 0.06 & 2.39 \\ 
  Sentence (Negative)$\times$Experiment (Exp. 2)& 0.27 & 0.07 & 3.99 \\ 
  Window (Late)$\times$Experiment (Exp. 2)  & 0.15 & 0.05 & 2.82 \\ 
  Age (Three-year-olds)$\times$Sentence (Negative)$\times$Window (Late) & -0.15 & 0.07 & -2.25 \\ 
  Age (Four-year-olds)$\times$Sentence (Negative)$\times$Window (Late)  & -0.15 & 0.07 & -2.05 \\ 
  Age (Adults)$\times$Sentence (Negative)$\times$Window (Late)  & -0.10 & 0.08 & -1.34 \\ 
  Age (Three-year-olds)$\times$Sentence (Negative)$\times$Experiment (Exp. 2)  & -0.34 & 0.09 & -3.79 \\ 
  Age (Four-year-olds)$\times$Sentence (Negative)$\times$Experiment (Exp. 2)  & -0.40 & 0.09 & -4.24 \\ 
  Age (Adults)$\times$Sentence (Negative)$\times$Experiment (Exp. 2) & -0.34 & 0.10 & -3.34 \\ 
  Age (Three-year-olds)$\times$Window (Late)$\times$Experiment (Exp. 2)  & -0.11 & 0.07 & -1.54 \\ 
  Age (Four-year-olds)$\times$Window (Late)$\times$Experiment (Exp. 2)  & -0.16 & 0.08 & -2.00 \\ 
  Age (Adults)$\times$Window (Late)$\times$Experiment (Exp. 2)  & -0.11 & 0.08 & -1.44 \\ 
 Sentence (Negative)$\times$Window (Late)$\times$Experiment (Exp. 2)  & -0.17 & 0.07 & -2.34 \\ 
  Age (Three-year-olds)$\times$Sentence (Negative)$\times$Window (Late)$\times$Experiment (Exp. 2)  & 0.40 & 0.10 & 4.12 \\ 
  Age (Four-year-olds)$\times$Sentence (Negative)$\times$Window (Late)$\times$Experiment (Exp. 2)  & 0.52 & 0.10 & 4.94 \\ 
  Age (Adults)$\times$Sentence (Negative)$\times$Window (Late)$\times$Experiment (Exp. 2) & 0.22 & 0.11 & 2.07 \\ 
   \hline
\end{tabular}
\end{center}
\end{table}
...
To compare the results of Experiment 1 to those of Experiment 2, we combined the data for both experiments and fit a linear mixed-effects model exploring the interactions between sentence type, age group, time window, and experiment on proportion looking to the target (Table \ref{tab:bothmodel}).\footnote{The model specification was \texttt{on.target $\sim$ age~$\times$~sentence~$\times$~window~$\times$~experiment + (sentence + window~\textbar~subject) +  (sentence + age + window~\textbar~item)}.} The three-way interactions between age, sentence type, and experiment indicate that performance on negative sentences was \emph{lower} for three-year-olds ($\beta = - .34$, $p  < .001$), four-year-olds ($\beta = -.40$, $p <  .001$), and adults ($\beta = -.34$, $p  < .001$) on Experiment 2.  Examining Figure \ref{fig:e2simple}, this pattern appears to be due to children's looks to the incorrect picture in the initial period and adults' slower latency to target in response to negative sentences.  The delay in identifying the target leads to overall lower performance in response to negative sentences in Experiment 2.  In the 4-way interactions, however, which include the effect of time window, we now see \emph{increased} late performance on negative sentences for three-year-olds ($\beta = .40$, $p < .001$), four-year-olds ($\beta = .52$, $p < .001$), and adults ($\beta = .22$, $p <  .05$) in Experiment 2.  Thus, although target looking in Experiment 2 was lower than performance on Experiment 1 in the earlier window of time, in the later window of time performance increased such that three-year-olds now showed comprehension of negative sentences, and four-year-olds showed improved accuracy.  

Although adults in Experiment 2 appeared slightly slower to process negations than they were in Experiment 1, a model fit only to the adult data did not find a significant interaction between sentence type, time window, and experiment (Sentence (Negative)$\times$Window (Early)$\times$Experiment (Exp. 2); $\beta = -.05$, $p <  .37$).\footnote{The model specification was \texttt{on.target $\sim$ sentence~$\times$~window~$\times$~experiment + (sentence~$\times$~window~\textbar~subject) +  (sentence + window~\textbar~item)}.}  This model of only the adult data also failed to show a significant main effect of negation ($\beta = -.03$, $p < .38$).  This indicates that adults in our experiment were not significantly worse at responding to negative sentences compared to positive sentences, although the direction of the coefficients does suggest a potential slight cost of negation, particularly in the early window of Experiment 2.   

Although children heard the exact same sentences in Experiments 1 and 2, their comprehension appeared different in these different contexts.  Two-year-olds struggled to orient towards the target picture in both contexts, even in Experiment 2 when there was no baseline preference for either item.  Three-year-olds failed to show robust comprehension of negation in a context where the negative sentences referred to the absence of an object, but showed some late comprehension when negative sentences referred to an alternative object.  Four-year-olds showed comprehension of negation in both experiments, but like 3-year-olds, their comprehension was delayed in Experiment 2 when the negative sentence referred to an alternative object.  These results suggest that different contexts of negation --- in these experiments either a target with an absent feature, or a target with an alternative feature --- lead children to process these sentences in very different ways.  
...
The study of negation has been used to inform our understanding of sentence processing more broadly.   Many studies of adults' comprehension of negation have suggested that negative sentences are challenging to process even for adult language users.  This work has been used to explore a number of possible ways that we represent language, e.g. as propositions \cite<e.g.>{hclark1972} or as simulated representations \cite<e.g.>{kaup2003}.  Furthermore, the finding that an appropriate context can alleviate processing demands \cite<e.g.>{wason1965} supports research on pragmatics and the role of context on linguistic processing. Just as these studies of adults' comprehension of negation have helped us understand adult sentence processing more generally, we hope that exploring children's comprehension of negative sentences will advance our understanding of how children process complex sentences.

We conducted a study of children's comprehension of negation in which negative sentences referred either to an absent item (Experiment 1) or an alternative item (Experiment 2).  We proposed several factors that might influence children's acquisition of negation, including linguistic factors (which were held constant in these experiments), conceptual factors, and pragmatic factors.  If conceptual factors were of primary importance in children's negation comprehension, Experiment 2 should have been more difficult for children, since it required them to go beyond simply matching ``no'' to non-existence in general and identify a specific non-existence (``no apples'').  Instead, children showed a different pattern of comprehension in Experiment 2, with three- and four-year-olds looking away from the target object early in the trial, but improved comprehension several seconds later.  We propose that contextual factors are the best explanation for the different processing patterns seen in these experiments, with children interpreting the negative sentence differently depending on the context in which the sentence was presented.  

We initially expected that children would be successful at comprehending negative sentences in Experiment 1, because these sentences represented ``nonexistence negation'', i.e. a negative sentence which refers to the absence of an object, which emerge early in children's production \cite{pea1980, bloom1970}.  How do our contexts in Experiment 1 differ from the contexts in which children produce nonexistence negation?  Children's production of nonexistence often occurs in feeding situations, where a child has recently finished their food or drink (e.g. ``no juice'', Bloom 1970), or in situations where an object is not in its expected location or an expected action fails to occur.  In fact, in his investigation of children's early negative utterances, \citeA{pea1980} splits nonexistence into two categories, disappearance and unfulfilled expectations.  Both of these types of negation would occur in contexts where the absence of an item is highly salient, either because it has recently disappeared or an expectation has been violated, unlike our stimuli, which merely comment on the absence of a property.  Although the context slides presented in this experiment have been shown to set up a strong expectation in adults (e.g. that boys have apples, Nordmeyer \& Frank, under review), it is possible that these contexts were too subtle to influence children's sentence processing.  Future work will explore what kinds of contexts are most effective at facilitating children's comprehension of negation.  

We observed a striking phenomenon in the time-course of processing negative sentences: On hearing the target noun, three- and four-year-old-children oriented towards the matching picture even when the noun was negated.  There are a number of possible explanations for this pattern.  One possibility is that this context is also pragmatically infelicitous.  In a context where two characters have an identifying feature (e.g. the objects that they are holding), it is more felicitous to describe either character by labeling that feature.  For example, in the example shown in Figure 4, children may be expecting to hear either ``apples'' or ``presents'' to describe the boys; ``no apples'' is an odd choice because it is a more costly utterance (requiring an additional word) and does not specify what the target character \emph{is} holding.  However, it is not clear why this would cause children to look \emph{away} from the target character as opposed to simply taking longer to orient to the correct character.  Another explanation is that it takes children longer to fully integrate the negative element (``no'') with the referent (``apples''), even though they understand the meaning of the word ``no''. (CITE??) This could explain the tendency to shift away from the target upon hearing the disambiguating noun, as well as very young children's difficulty with our task in both conditions (e.g. 2-year-olds may not be able to integrate the negative element in the time provided them here).  Finally, it is possible that when children are presented with two equally likely alternatives, identifying the referent of negation requires ruling out the named object.  That is, when children hear the phrase ``no apples'', they may try to first find the boy \emph{with} apples in order to rule him out as a possible target; only then do they turn to the boy with presents.  This would explain why children systematically orient away from the target character, even if they are fixating on the target when they hear the disambiguating noun.

Another possibility is that inhibitory control is playing a role in both Experiment 1 and Experiment 2.  In his investigation of children's early negative utterances, \citeA{pea1980} identified the development of self-prohibition as a critical step in children's acquisition of negation.  Although Pea was particularly interested in cases where a child succeeds in inhibiting their own actions after producing a self-prohibition, the opposite case is equally interesting in light of the results of the present studies.  Children who utter the word ``no'' while reaching towards a desired but forbidden object may understand the meaning of the word ``no,'' and understand that they are not supposed to touch what they are reaching for.  But in many cases these children still proceed with the prohibited action; although they understand the concept of prohibition, they do not have the inhibitory control necessary to stop their own actions. In our experiments, children may have be compelled to look to pictures that match named nouns, even when that noun has been negated. Under this explanation, a child who hears the phrase ``no apples'' will look to the picture of apples, and it may take children several seconds to orient to the correct picture.  It may be easier to inhibit this response in Experiment 2, when the correct alternative is equally salient.  This could explain three-year-old's poor performance in Experiment 1, but their improved performance after a delay in Experiment 2.  It is likely that certain naturalistic contexts alleviate inhibitory demands; for example, when a salient object disappears, or a strongly held expectation is violated.  

Our current work suggests a possible synthesis of previous adult work on comprehension and developmental work on production.  In our experiments, children displayed a very different pattern of comprehension depending on the context in which the same sentence was presented.  Similarly, adults show faster comprehension when negative sentences are presented in a supportive context \cite<e.g.>{wason1965}. Further exploration of the role of pragmatics in negative sentence processing and the specific contexts which make negation salient for children is essential to understanding the development of children's comprehension of negation.  
